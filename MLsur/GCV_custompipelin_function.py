...
class MutfreqFtSel(BaseEstimator, TransformerMixin):
    ...
    # initializer
    def __init__(self, mini_count=2):
    ...
    def transform(self, X, y=None):
        ...
        X2 = X.loc[
            :,
            (self.mutfreq >= self.mini_count / self.counts)
            & (self.mutfreq <= (self.counts - self.mini_count) / self.counts),
        ]
...
class KeepMSinList(BaseEstimator, TransformerMixin):
    """
    Custom transformer class that selects columns from a DataFrame based on a provided list of mutation sequences.

    Parameters
    ----------
    MS_list : list
        A list of mutation sequences to keep in the transformed DataFrame.

    Attributes
    ----------
    MS_list : list
        The provided list of mutation sequences.
    MS_intersection : pd.Index
        The intersection of the columns in the transformed DataFrame with the `MS_list`.
    feature_names_out_ : pd.Index
        The names of the columns in the transformed DataFrame.

    Methods
    -------
    fit(self, X: pd.DataFrame, y=None)
        Calculates the intersection of the columns in `X` with the `MS_list` and stores it in `self.MS_intersection`.
    transform(self, X: pd.DataFrame, y=None)
        Checks if the class has been fitted (i.e., `fit` has been called). Then, it returns a DataFrame (`X`) with only the columns that intersect with `self.MS_intersection`.
    get_feature_names_out(self)
        Returns the names of the columns in the transformed DataFrame.
    """

    def __init__(self, MS_list=None):
        self.MS_list = MS_list

    def fit(self, X: pd.DataFrame, y=None):
        if self.MS_list is None:
            raise ValueError(
                "MS_list is None. Please provide a valid list of mutation sequences."
            )

        # calculate the missing data (NA) rate per columns
        self.MS_intersection = X.columns.intersection(set(self.MS_list))
        return self

    def transform(self, X: pd.DataFrame, y=None):
        check_is_fitted(self, ["MS_intersection"])

        X = X[self.MS_intersection]  # return Dataframe with NA rate lower than NAmax
...
class MutfreqFtSelDelsize(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        X2 = X.loc[
            :,
            (self.mutfreq >= self.mini_count / self.counts)
            & (self.mutfreq >= 1 - self.mini_count / self.counts),
        ]
        self.feature_names_out_ = X2.columns
        return X2
...
class FilterNA(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        X = X.loc[
            :, self.narate <= self.NAmax
        ]  # return Dataframe with NA rate lower than NAmax
...
class SimpleImputerMostFreq(SimpleImputer):
    ...
    def transform(self, X):
        ...
        if isinstance(X, (pd.DataFrame, pd.Series)):
            return pd.DataFrame(
                data=X2, columns=self.get_feature_names(), index=X.index
            )
        else:
            return X2
...
class SelectKBestWithName(SelectKBest):
    ...
    def transform(self, X):
        ...
        if isinstance(X, (pd.DataFrame, pd.Series)):
            return pd.DataFrame(
                data=X2, columns=super().get_feature_names_out(), index=X.index
            )
        else:
            return X2
...
class SelectKBestCox(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        ...
        """
        if self.random:
            self.feature_names_out_ = X.sample(n=self.k, axis="columns").columns
            self.score_ = None
            return self
            ...
            for j in range(n_features):
                ...
                if self.pvalue:
                    try:
                        scores[j] = compare_survival(
                            y[np.argwhere(nabool)[:, 0]], Xj[nabool]
                        )[1]
                    ...
                    try:
                        ...
                        scores[j] = m.score(
                            Xj[nabool].reshape(-1, 1), y[np.argwhere(nabool)[:, 0]]
                        )
                        succes.append(X_col[j])
                    except:
                        error.append(X_col[j])
            ...
            if self.pvalue:
                self.feature_names_out_ = self.score_.sort_values(ascending=True)[
                    : self.k
                ].index
                return self
                ...
                self.feature_names_out_ = self.score_.sort_values(ascending=False)[
                    : self.k
                ].index
                return self
...
class SelectKBestCox_invert_mut(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X):
        ...
        if self.random:
            ...
            return X[K_best_col].apply(
                lambda x: invertcol01(x) if x.name in self.inverted_col_ else x, axis=0
            )
...
def impute_sample(chridseries, random_st=None):
    ...
    chridseries.loc[nullindex] = (
        chridseries.dropna().sample(nullindex.sum(), random_state=random_st).values
    )
    return chridseries


def impute_mean(chridseries, random_st=None):
    nullindex = chridseries.isnull()
    chridseries.loc[nullindex] = chridseries.dropna().mean()
    return chridseries


def impute_most_frequent(chridseries, random_st=None):
    nullindex = chridseries.isnull()
...
class impute_rnd(BaseEstimator, TransformerMixin):
    """
    Class sklearn compatible pour imputer les données manquantes sur la base de la fréquence de mutation.
    __init__: Initializes the class with the chosen imputation method.
    fit: Marks the transformer as fitted.
    transform: Applies the specified imputation method to the input data.
    """

    def __init__(self, applied_method=impute_sample, random_st=None):
        """
        Initialize the object with the given applied method and random state.

        Parameters:
            applied_method (function): The method to be applied.
            random_st (object): The random state for the method.
    ...
    def transform(self, X, y=None):
        ...
        X.apply(lambda x: self.applied_method(x, self.random_st))
        return X
...
def minimum_nb_samples_with_value(counts_df, smallestGroupSize=3, thr=10, drop__col=0):
    ...
    gene_to_keep = (
        counts_df.apply(lambda row: (row >= thr).sum(), axis=0) >= smallestGroupSize
    )
